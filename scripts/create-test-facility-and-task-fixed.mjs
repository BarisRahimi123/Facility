...
async function createTestData() {
  console.log('🔧 Creating test facility and maintenance task...\n')

  try {
    // Get master admin user and organization
    ...
    console.log(`   Organization ID: ${users.organization_id}`)
    ...
    if (facilityError || !existingFacilities || existingFacilities.length === 0) {
      ...
      const facilityData = {
        ...
        status: 'active',
        capacity: 100,
        amenities: ['WiFi', 'Parking', 'HVAC', 'Security'],
        created_by: users.id,
    ...
    if (existingInvitation) {
      console.log(`✅ Token ${token} already exists! Connected to task: ${existingInvitation.task_id}`)
