...
DROP FUNCTION IF EXISTS public.send_user_invitation(p_email text, p_role user_role, p_invited_by uuid, p_facility_id uuid);

-- Drop any other versions that might exist
DROP FUNCTION IF EXISTS public.send_user_invitation(text, text, uuid);
DROP FUNCTION IF EXISTS public.send_user_invitation(text, text, uuid, uuid);
DROP FUNCTION IF EXISTS public.send_user_invitation(text, text, uuid, uuid, uuid);
DROP FUNCTION IF EXISTS public.send_user_invitation(text, text, uuid, uuid, uuid, jsonb);

-- Create the new function using text for role (more flexible)
...
BEGIN
  ...
  -- Create invitation - cast text to user_role if needed
  INSERT INTO user_invitations (
    ...
    p_email,
    p_role::text,  -- Ensure it's text, will be cast to user_role if column is enum
    p_invited_by,
    p_facility_id,
    p_organization_id,
    v_token,
...
-- Also create/update accept_invitation
...
BEGIN
  ...
  INSERT INTO users (
    ...
    v_invitation.role::text,  -- Cast to text in case role column is enum
