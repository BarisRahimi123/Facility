-- Drop all versions of send_user_invitation using dynamic SQL
DO $$
DECLARE
    func_record RECORD;
BEGIN
    -- Loop through all versions of send_user_invitation
    FOR func_record IN 
        SELECT 
            'DROP FUNCTION IF EXISTS ' || n.nspname || '.' || p.proname || '(' || 
            pg_catalog.pg_get_function_identity_arguments(p.oid) || ');' as drop_command
    ...
    LOOP
        ...
        RAISE NOTICE 'Dropped: %', func_record.drop_command;
    END LOOP;
    
    -- Loop through all versions of accept_invitation
    FOR func_record IN 
        SELECT 
            'DROP FUNCTION IF EXISTS ' || n.nspname || '.' || p.proname || '(' || 
            pg_catalog.pg_get_function_identity_arguments(p.oid) || ');' as drop_command
...
-- Now create the correct version of send_user_invitation
...
-- Create the correct version of accept_invitation
