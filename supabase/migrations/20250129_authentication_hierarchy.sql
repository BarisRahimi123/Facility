-- Authentication Hierarchy Migration
-- Purpose: Set up proper user roles and invitation system for hierarchical user management

-- First, ensure we have the proper user roles enum
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'user_role') THEN
        CREATE TYPE user_role AS ENUM (
            'master_admin',      -- Master admin (district-wide authority)
            'sub_master',        -- Sub-master admin (facility-level authority)
            'staff',             -- Facility staff member
            'manager',           -- Facility manager
            'coordinator',       -- Facility coordinator
            'maintenance',       -- Maintenance staff
            'vendor',            -- External vendor
            'renter'            -- Public user who rents facilities
        );
    END IF;
END$$;

-- Update users table to use the new role enum
ALTER TABLE users 
...
ALTER COLUMN role TYPE user_role USING 
    CASE role 
        WHEN 'district_approver' THEN 'master_admin'::user_role
        WHEN 'site_approver' THEN 'sub_master'::user_role
        WHEN 'staff' THEN 'staff'::user_role
        WHEN 'manager' THEN 'manager'::user_role
        WHEN 'coordinator' THEN 'coordinator'::user_role
        WHEN 'maintenance' THEN 'maintenance'::user_role
