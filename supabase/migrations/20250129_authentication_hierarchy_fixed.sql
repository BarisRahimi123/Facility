-- Authentication Hierarchy Migration (Fixed)
-- Purpose: Set up proper user roles and invitation system for hierarchical user management

-- First, let's see what roles currently exist and add new ones if needed
DO $$ 
BEGIN
    -- Add new role values to existing enum if they don't exist
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'master_admin' AND enumtypid = 'user_role'::regtype) THEN
        ALTER TYPE user_role ADD VALUE IF NOT EXISTS 'master_admin';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'sub_master' AND enumtypid = 'user_role'::regtype) THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'renter' AND enumtypid = 'user_role'::regtype) THEN
