...
console.log('Main Profile page component started rendering at:', startTime);
...
export default function ProfilePage() {
  console.log('Main Profile page function execution started at:', performance.now() - startTime, 'ms');
  ...
  useEffect(() => {
    const checkSupabase = async () => {
      try {
        ...
        const { data, error } = await supabase.from('facilities').select('count').limit(1).single();
        ...
        setSupabaseError('Failed to connect to Supabase');
      }
    };
  ...
  if (!user) {
    return <div className="flex justify-center items-center h-screen">Please sign in to view this page.</div>;
  }
