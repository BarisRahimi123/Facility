...
export default function AssignStaffToFieldModal({
  ...
  const loadData = async () => {
    try {
      ...
      if (assignmentsResponse.error) {
        ...
        // Filter assignments for this specific field
        const fieldAssignments = assignmentsResponse.data?.filter(
          assignment => assignment.field_id === fieldId
        ) || [];
        setAssignments(fieldAssignments);
      }
    } catch (error) {
      toast({
  ...
  const handleDeleteAssignment = async (assignmentId: string) => {
    try {
      const response = await deleteFieldAssignment(assignmentId);
            ...
            <div>
              ...
              {assignments.length === 0 ? (
                <p className="text-sm text-muted-foreground py-4 text-center border rounded-lg">
                  No staff currently assigned to this field
                </p>
              ) : (
