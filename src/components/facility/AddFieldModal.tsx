...
export function AddFieldModal({ isOpen, onClose, onSubmit, facilityId }: AddFieldModalProps) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      setShowSuccess(true);
      setLastCreatedField(fieldData.name);
    } catch (error) {
      ...
      <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto bg-background border-border text-foreground shadow-2xl">
        ...
        {showSuccess ? (
          <div className="py-8">
            ...
            <h3 className="text-xl font-semibold text-center mb-2">{lastCreatedField} has been created!</h3>
            <p className="text-center text-muted-foreground mb-8">
              The field is now available for bookings and reservations.
            </p>
            <div className="flex justify-center space-x-4">
              <Button
                variant="outline"
                onClick={handleCloseModal}
                className="border-border text-muted-foreground hover:bg-accent hover:text-accent-foreground min-w-[120px]"
              >
                Close
              </Button>
              <Button
                onClick={handleCreateAnother}
                className="bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary text-primary-foreground shadow-lg hover:shadow-xl min-w-[160px]"
              >
