...
import { getFolders, getSubfolders, getFolderDocuments, deleteFolder, type DocumentFolder } from '@/app/actions/documentFolders';
...
interface FacilityDocumentsWithFoldersProps {
  facilityId: string;
  documents: Document[];
...
export function FacilityDocumentsWithFolders({ facilityId, documents, onDocumentsChange }: FacilityDocumentsWithFoldersProps) {
  ...
  const isPDF = (fileType: string, fileName: string, fileUrl?: string) => {
    const t = (fileType || '').toLowerCase();
    const n = (fileName || '').toLowerCase();
    const u = (fileUrl || '').toLowerCase();
    return t.includes('pdf') || n.endsWith('.pdf') || /\.pdf(\?|$)/.test(u);
  };

  // Handle document viewing - use PDF viewer for PDFs, external link for others
  const handleViewDocument = (doc: Document) => {
    if (isPDF(doc.file_type, doc.file_name, doc.file_url)) {
  ...
  const loadFoldersAndDocuments = async () => {
    ...
    try {
      if (currentFolder) {
        // Load subfolders and documents for current folder
        const [subfolderData, folderDocData] = await Promise.all([
          getSubfolders(currentFolder.id),
          getFolderDocuments(currentFolder.id)
        ]);
        setFolders(subfolderData);
  ...
  return (
    <div className="space-y-6">
            ...
            <div>
              ...
              <div className={`grid gap-4 ${viewMode === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : 'grid-cols-1'}`}>
                {folders.map((folder) => (
                  <Card 
                    key={folder.id} 
                    className="bg-card/50 border-border hover:bg-card cursor-pointer transition-all duration-200 hover:shadow-sm group"
                  >
                      ...
                      <div className="flex items-start justify-between h-full">
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0 space-y-1">
                            <p className="font-medium text-foreground group-hover:text-primary transition-colors truncate leading-tight">
                              {folder.name}
                            </p>
                            ...
                            <div className="flex items-center gap-2 pt-1">
                              <span className="text-xs text-muted-foreground">
                                {(folder as any).document_count || 0} documents
                              </span>
                              {(folder as any).subfolder_count && (folder as any).subfolder_count > 0 && (
                                <>
                                  <span className="text-xs text-muted-foreground">•</span>
                                  <span className="text-xs text-muted-foreground">
                                    {(folder as any).subfolder_count} folders
                                  </span>
                                </>
                              )}
                            </div>
            ...
            <div>
              ...
              <div className={`grid gap-4 ${viewMode === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
                {filteredDocuments.map((doc) => (
                  <Card 
                    key={doc.id} 
                    className={`bg-card/50 border-border hover:bg-card transition-all duration-200 hover:shadow-sm group ${isPDF(doc.file_type, doc.file_name, doc.file_url) ? 'cursor-pointer' : ''}`}
                    onClick={() => isPDF(doc.file_type, doc.file_name, doc.file_url) && handleViewDocument(doc)}
                  >
                    <CardContent className="p-4">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="bg-popover border-border">
                            ...
                            >
                              ...
                              {isPDF(doc.file_type, doc.file_name) ? 'View PDF' : 'View'}
                            </DropdownMenuItem>
                            <DropdownMenuItem 
                              className="text-foreground focus:bg-accent focus:text-accent-foreground"
                              onClick={() => {
          ...
          {folders.length === 0 && filteredDocuments.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">
                {currentFolder 
                  ? 'Start organizing by creating subfolders or uploading documents.'
      ...
      {/* Embedded viewer is handled above via viewingPDF */}
    </div>
  );
}
